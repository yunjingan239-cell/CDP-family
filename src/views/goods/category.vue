...
const {
  ...
  handleQuery,
  handleDelete,
} = useTablePage<GoodsCategoryType, GoodsCategoryQuery>({ getPageList, deleteById }, emit);

// 自定义 handleAdd 方法
const handleAdd = () => {
  editRef.value?.open('add', '新增分类', {});
};

// 自定义 handleEdit 方法
const handleEdit = (row: any) => {
  editRef.value?.open('edit', '编辑分类', { ...row });
};
</script>
  ...
  <div class="layout-padding">
    ...
    <el-form :inline="true" :model="query" label-width="80px" label-suffix=":">
      <el-form-item label="分类名称">
        <el-input v-model="query.name" placeholder="请输入分类名称" clearable maxlength="30" />
      </el-form-item>
      <el-form-item>
    ...
    <!-- 分页 -->
    <Page :page="page" @pageChange="queryData" />
    
    <!-- 新增/编辑弹窗 -->
    <category-edit ref="editRef" @refresh="queryData" />
  </div>
</template>
