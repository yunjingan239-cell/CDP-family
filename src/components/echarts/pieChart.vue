...
import { ref, reactive, markRaw, onMounted, onBeforeUnmount, watch, nextTick, onActivated } from "vue";
...
const state = reactive({
    nvl: ['', null, undefined], // 用于判断图表实例是否存在
...
function initChart() {
    // 实例存在则移除
    if (!state.nvl.includes(state.chart)) state.chart.dispose();
    // 初始化echarts实例, 参数2: 暗黑主题 'dark'
    // 要使用 `markRaw` 返回该对象本身, 而不是转为代理对象返回. 防止 echarts 有些功能失效(如: tooltip)
